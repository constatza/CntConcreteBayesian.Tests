...
namespace MGroup.Stochastic.Tests
{
	public class ConcreteHierarchicalBayesian
	{
		...
		public static void RunSimulation()
		{
			...
			(var cementSampler, var mortarSampler, var concreteSampler) = InitializeSimulation();

			var numParameterSamples = 1000;
			...
			double HyperparameterLikelihoodModel(double[] sample)
			{
				...
				return Math.Log(cementLikelihoodValue) + Math.Log(mortarLikelihoodValue) + Math.Log(concreteLikelihoodValue);
			}
		}
		...
		private static (IProbabilityDistributionSampler cementSampler, IProbabilityDistributionSampler mortarSampler, IProbabilityDistributionSampler concreteSampler) InitializeSimulation()
		{
			...
			return (cementSampler, mortarSampler, concreteSampler);
		}
